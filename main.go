...
// ProxySouece 구조체는 프록시를 가져올 Source와 Parser 함수로
// 이루어져 있습니다. Source는 여러 개일 수 있으나, 단일한 Parser 함수로
// 호환 가능한 형태여야 합니다. Parser는 func(source string) []*url.URL 형태를
// 충족시켜야 합니다.
type ProxySouece struct {
	Source []string
	Parser func(source string) []*url.URL
}

// ProxyFactory 구조체는 복수의 ProxySouece를 가집니다.
type ProxyFactory struct {
	ProxySources []*ProxySouece
}

var defaultProxySources = []*ProxySouece{{
	Source: []string{
...
// NewProxySource 함수는 빈 ProxySouece 구조체를 반환합니다.
func NewProxySource() *ProxyFactory {
	return &ProxyFactory{}
}

// AddDefaultOpenProxySources 함수는 패키지에 포함되어 있는
// 기본 ProxySouece들을 추가합니다.
func (pf *ProxyFactory) AddDefaultOpenProxySources() {
	for _, defaultProxySource := range defaultProxySources {
		pf.ProxySources = append(pf.ProxySources, defaultProxySource)
	}
}

// AddCustomProxySource 함수는 사용자가 구현한 ProxySouece를
// 인자로 받아 ProxyFactory 객체에 추가합니다.
func (pf *ProxyFactory) AddCustomProxySource(proxySouece *ProxySouece) {
	pf.ProxySources = append(pf.ProxySources, proxySouece)
}
...
func (pf *ProxyFactory) ProxySource() chan *url.URL {
	...
	go func() {
		...
		for _, proxySource := range pf.ProxySources {
			for _, source := range proxySource.Source {
				wg.Add(1)
				go func(source string) {
					defer wg.Done()
					for _, proxy := range proxySource.Parser(source) {
						bufCh <- proxy
					}
				}(source)
			}
